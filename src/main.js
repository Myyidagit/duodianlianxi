...
Vue.use(Mint,{
  lazyload:{
    error:"https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1505888372456&di=874cabc569948af0cae5d5e74e868c96&imgtype=0&src=http%3A%2F%2Fwww.518z.com%2Fuploads%2Fallimg%2F160120%2F1G01411A-0.png",
    loading:"http://img.zcool.cn/community/01cddc57a93f670000018c1bff1e12.gif",
    threshold: 50,
    effectFadeIn: true
  }
});
...
const aa = store.state.bb
router.beforeEach((to, from, next) => {
	if (to.path == '/vip') {
		store.commit('CHANGE_STATUS',false);
		console.log('进入vip时的状态'+aa)
		next()
		return
	}
	if (to.path != '/vip') {
		const cc = store.state.bb
		if(!cc){
			console.log("想去别的页面？想也别想")
			next({path:"/vip"})
		}else{
			console.log("出去了"+aa)
			next()
